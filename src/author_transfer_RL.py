...
def main(_):
    ...
    generator, rnnlm, style_discriminator, siamese_discrim, semantic_discriminator, rollout, vocab, tsf_vocab_inv = \
...
#    print("pretrain a new model...")
    ...
    generator_ts_batches = data_helpers.batch_iter(list(zip(tsf_encoder_sents, orig_words, tsf_encoder_sent_len)), FLAGS.batch_size, FLAGS.epochs)
    batch_count = 0
    dev_size = len(dev_orig_sents)
    for batch,batch_ts in itertools.zip_longest(generator_batches,generator_ts_batches):
        # load data
        batch_orig_sents, batch_orig_words, batch_orig_sent_len = zip(*batch)
        batch_tsf_sents, _ , batch_tsf_sent_len = zip(*batch_ts)
        ...
        rewards = rollout.get_reward(sess, generator, batch_orig_sents, batch_orig_words, batch_orig_sent_len, \
                                     batch_tsf_sents , batch_tsf_sent_len,\
                                     generator_outputs, rnnlm, style_discriminator,siamese_discrim, semantic_discriminator, \
        ...
        if (batch_count % 100 == 0):
            ...
            for itera in range(int(dev_size/FLAGS.batch_size)):
                ...
                if start_ind + FLAGS.batch_size >= len(tsf_encoder_sents):
                    start_ind = 0
                    end_ind = FLAGS.batch_size
                target_sents = tsf_encoder_sents[start_ind:end_ind]
                target_lens  = tsf_encoder_sent_len[start_ind:end_ind]
                # beam search
                ...
                batch_style_reward, batch_sem_reward, batch_lm_reward, batch_siamese_reward, batch_reward = \
                                    rollout.get_sent_reward(sess, FLAGS.batch_size, batch_orig_words, \
                                                            ...
                                                            target_sents, target_lens, \
                                                            rnnlm, style_discriminator, semantic_discriminator,siamese_discrim, False)
            ...
            avg_dev_siamese_reward = np.mean(dev_siamese_rewards)
