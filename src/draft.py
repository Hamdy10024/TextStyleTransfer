...
def generatePretrainSiameseSamples(orig_sents, tsf_sents, orig_sent_len, tsf_sent_len, first_ind, last_ind):
    orig_sents = np.expand_dims(np.array(list(orig_sents[first_ind:last_ind])),axis = 0)
    tsf_sents = np.expand_dims(np.array(list(tsf_sents[first_ind:last_ind])),axis = 0)
    combined = np.append(orig_sents, tsf_sents, axis = 0)
    orig_sent_len = np.expand_dims(np.array(list(orig_sent_len[first_ind:last_ind])),axis = 0)
    tsf_sent_len = np.expand_dims(np.array(list(tsf_sent_len[first_ind:last_ind])),axis = 0)
    combined = np.append(orig_sents, tsf_sents, axis = 0)
    combined_len = np.append(orig_sent_len, tsf_sent_len, axis = 0)
    return combined, combined_len

def batch_double_iter(orig_data, tsf_data, orig_sent_len, tsf_sent_len, batch_size, num_epochs):
    data_size = 176787#len(tsf_data)
    for epoch in range(num_epochs):
        shuffle_indices = np.random.permutation(np.arange(data_size))
        orig_data = np.array(orig_data)[shuffle_indices]
        tsf_data = np.array(tsf_data)[shuffle_indices]
        orig_sent_len = np.array(orig_sent_len)[shuffle_indices]
        tsf_sent_len =  np.array(tsf_sent_len)[shuffle_indices]
        start_index = 0
        while start_index < data_size:
            ...
            yield generatePretrainSiameseSamples(orig_data, tsf_data, orig_sent_len, tsf_sent_len,start_index, end_index)
